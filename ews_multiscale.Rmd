...
morans_over_time <- morans_over_time %>%
  mutate(
    runmean_two_year = zoo::rollmean(morans_i, k = 52, align = "right", fill = NA)
  )
...
plot(morans_kendall$kens_tau, peak_cases$peak[3:(nrow(peak_cases))])
summary(lm(peak_cases$peak[3:(nrow(peak_cases))] ~ morans_kendall$kens_tau))
```
