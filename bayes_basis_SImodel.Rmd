...
```{r susceptible-reconstruction}
# Calculate cumulative cases and cumulative births
niamey_pop_merge <- niamey_pop_preds %>%
  spread(variable, value) %>%
  rename(
    pop_size = `Population Size`,
    num_births = `Weekly Births`,
    year = Year
  )

measles_recon <- measles_dat %>%
  mutate(
    year = year(date)
  ) %>%
  left_join(niamey_pop_merge) %>%
  mutate(
    cum_cases = cumsum(cases),
    cum_births = cumsum(num_births)
  )

# Estimate residuals from cum_cases ~ cum_births regression
susc_dynamics <- resid(gam(cum_births ~ s(cum_cases), data = measles_recon))

susceptible_deviations <- tibble(
  date = measles_recon$date,
  susc_dynamics = susc_dynamics
)

ggplot(susceptible_deviations, aes(x = date, y = susc_dynamics)) +
...
This model is called using the `rjags::jags.model()` function.
...
# Process model
for(t in 1:(num_obs-1)){
  # beta[t] <- mu[yr[t]] + inprod(W[wk[t],], eta[yr[t],])  # spline model for rate of transmission
  muI[t+1] <- max(1, log(beta[t]) + alpha*log(I[t]) + log(z[t] + S0))  # determinstic update for infections
  I[t+1] ~ dlnorm(muI[t+1], tau)  #, I[t])  # stochastic update for infections
  S[t+1] <- S[t] - I[t+1] + b[t]  # determinstic update for susceptibles
}  # end process loop

# Priors
I[1] <- I1  # initial number of infections, from data
S[1] <- S0 + z[1]  # initial number of susceptibles
S0 ~ dpois(100000)  # prior for mean susceptible size
alpha ~ dnorm(0, 0.001)
...
# for(iyr in num_years){
#   mu[iyr] ~ dnorm(0, 0.1)  # prior for mean in transmission spline
# 
#   for(iknot in 1:num_knots+1){
#     eta[iyr, iknot] ~ dnorm(0, 0.1)  # priors for coefficients in transmission spline
#   }  # end knot loop
# 
# }  # end year loop
...
jags_data <- list(
  y = measles_recon$cases+1,
  I1 = measles_recon$cases[1],
  z = round(susceptible_deviations$susc_dynamics),
  b = measles_recon$num_births,
  W = bases,
  p = 0.15,
  num_obs = nrow(measles_recon),
  num_years = length(unique(year(measles_recon$date))),
  num_knots = num_knots,
  yr = as.numeric(as.factor(year(measles_recon$date))),
  wk = as.numeric(as.factor(week(measles_recon$date)))
)
...
mcmc_results <- coda.samples(model, variable.names = c("I"), n.iter = 2000)
plot(measles_recon$cases)
lines(summary(mcmc_results)$stat[,1])
```
