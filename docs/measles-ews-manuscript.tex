...
Agadez, Maradi, Niamey, and Zinder (figure \ref{data-plot}A). The data
were collected over an 11 year period from 1995-2005 (figure
...
\subsection{Model-data comparisons}\label{model-data-comparisons}}
...
remaining time series into two windows of equal length (figure S2). The
...
(figure S3). EWS were then calculated for each interval.
...
decreases (figure \ref{scatters}B) {[}15{]}. Our model for Agadez
...
(figure \ref{emerge-aucs}C). Their performance declines as the amount of
...
depletion was relatively small (figure \ref{emerge-aucs}C).
...
the vaccine campaign increased. Again, this is because of shorter null
and test intervals.
