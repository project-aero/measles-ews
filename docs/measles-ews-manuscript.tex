...
transition {[}12--14{]}. These findings suggest that early warning
signals could be operationalized to develop early warning systems of
disease emergence, elimination, or outbreaks of endemic diseases.

However, operationalizing EWS, and deploying early warning systems based
on them, faces many challenges. For example, using EWS in an ``online
mode'' requires choosing temporal windows over which EWS are calculated.
These moving windows should be long enough to provide reliable
statistics, but short enough to ``forget'' the past so as to not
...
answer for the wrong reasons. That is, EWS might perform well for a
given time series, but for reasons having nothing to do with critical
slowing down. Indeed, without known the critical point (i.e., when
...
transmission. This means our conclusions are not based on one-off
events. Third, we can specifically simulate emergence and elimination
events. This means we can separate the stress testing of EWS from the
research necessary to operationalize EWS. Thus, we have all the
flexibility of a theoretical model, but we remain tethered to reality
because the model parameters are fitted to real data.
