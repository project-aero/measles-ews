...
n_{I,t} &\sim \text{Binomial}(\lambda_{I}, E_{t}) \\
n_{O,t} &\sim \text{Poisson}(\psi \times dt) \\
n_{R,t} &\sim \text{Binomial}(\lambda_{R}, I_{t}),
\end{align}
...
Only $\lambda_E$ varies over time.
...
We assume observed case reports ($\textbf{y}$) are drawn from a Negative Binomial distribution subject to a constant reporting fraction ($\rho$),
...
We seek to estimate 12 parameters for each city (Table 2).
Note that we set $\gamma = 365/14$ assuming a 2 week infectious period.
We assume $\gamma$ is known to improve estimates of other parameters and because the infectious period of measles is well studied.
...
+ **09-10-2018**
  ...
  - Likihood is now poisson. Initial fitting kept showing very little overdispersion, so we're going to use Poisson to speed up computation.
