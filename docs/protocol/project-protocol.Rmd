...
\begin{align}
dS &= b_t dt - \lambda_t S_t dt + dW_t \\
...
| ---------- | ------------------ | --------------- |
| birth | (1, 0, 0) | $N_t \mu$ |
| transmission | (-1, 1, 0) | $S(I+\psi) \beta_t / N_t$ |
...
2. Conduct a global parameter search using 500 parameter sets from above. The parameter sets within the Latin hypercube will serve as starting conditions for iterated filtering using the function `pomp::mif2()`.

3. Conduct a local parameter search using the the 50 parameter sets from the global search with the highest likelihoods. We will consider the parameter set with the highest likelihood at this stage to be the MLEs ($\hat{\theta}$).
...
We will calculate the temporal correlation of $[\text{EWS}(t), \mathcal{R}_E(t)]$ for all EWS with Kendall's $\tau$ using the R function `cor(..., method = "kendall")`.
As shown in Table 2, we expect all EWS to positively covary with $\mathcal{R}_E$ over time, a prediction we can test by calculating the correlations and their significance using the function `cor.test(..., method = "kendall")`.
