...
OK, question is, can the GAMs for EWS beat detection of a GAM for mean cases?

```{r}

# Functions for GAM fitting and comparison
fit_gam <- function(gam_df) {
  test_formula <- as.formula(
    value ~ s(week, by = outbreak_year, k = 4, bs = "fs")
  )
  mgcv::gam(formula = test_formula, data = gam_df)
}
...
smooth_diff <- function(model, newdata, f1, f2, var, alpha = 0.05,
  ...
  dif <- as.numeric(X %*% coef(model))
  se <- sqrt(rowSums((X %*% vcov(model, unconditional = unconditional)) * X))
  crit <- qt(alpha/2, df.residual(model), lower.tail = FALSE)
  upr <- dif + (crit * se)
  lwr <- dif - (crit * se)
  tibble(pair = paste(f1, f2, sep = '-'),
         diff = dif,
         se = se,
         upper = upr,
         lower = lwr)
...
# Loop over data and calculate EWS
city_ews_list <- list()
for(do_city in unique(measles_data$region)){
  tmp_ews_list <- list()
  for(do_year in unique(measles_data$year)){
    tmp_cases <- measles_data %>%
    ...
    tmp_ews_list[[as.character(do_year)]] <- tmp_stats
  }
  city_ews_list[[do_city]] <- tmp_ews_list
}


# Loop over data and fit and compare GAMs
all_ews <- tibble()
for(do_city in names(city_ews_list)){
  tmp_city_ews <- city_ews_list[[do_city]]
  
  all_years_ews <- tibble()
  for(do_year in names(tmp_city_ews)){
    tmp_year_ews <- tmp_city_ews[[as.character(do_year)]]
    this_years_ews <- tibble()
    
    for(do_ews in names(tmp_year_ews)){
      tmp_ews <- tibble(
        city = do_city,
        ews = do_ews,
        value = tmp_year_ews[[do_ews]],
        year = as.numeric(do_year),
        week = 1:52
      )
      this_years_ews <- bind_rows(this_years_ews, tmp_ews)
    }  # end ews loop
    
    all_years_ews <- bind_rows(all_years_ews, this_years_ews)
    
  }  # end year loop
  all_ews <- bind_rows(all_ews, all_years_ews)
}  # end city loop


# Fit GAMs for each city-EWS combination
ews_to_fit <- c("autocorrelation", "mean", "variance")
all_diffs <- tibble()
for(do_city in unique(all_ews$city)){
  for(do_ews in ews_to_fit){
    gam_data <- all_ews %>%
      filter(city == do_city) %>%
      filter(ews == do_ews) %>%
      left_join(filter(outbreak_ids, region == do_city), by = "year") %>%
      dplyr::select(value, week, outbreak_year) %>%
      mutate(outbreak_year = as.factor(outbreak_year))
    
    if(min(gam_data$value, na.rm = TRUE) >= 0){
      gam_data$value <- log(gam_data$value + 0.0001)
    }
    
    tmp_gam <- fit_gam(gam_df = gam_data)
    pdata <- expand.grid(week = 1:52, outbreak_year = c("TRUE", "FALSE"))
    tmp_diff <- smooth_diff(model = tmp_gam, newdata = pdata, 
                            f1 = "TRUE", f2 = "FALSE", var = "outbreak_year")
    tmp_diff <- tmp_diff %>%
      mutate(
        city = do_city,
        ews = do_ews,
    ...
    all_diffs <- bind_rows(all_diffs, tmp_diff)
  }
}

ggplot(all_diffs, aes(x = week, y = diff)) +
