...
ggplot(filter(measles_data, week_of_year %in% 30:52), 
       aes(x = week_of_year, y = cases, group = season_year)) +
  geom_line(aes(color = outbreak_year)) +
...
feature_data <- all_ews %>%
  ...
  filter(ews_metric %in% c("autocorrelation", "variance", "mean")) %>%
...
for(do_region in regions){
  ...
  mod1 <- neuralnet(formula = nnet_formula, data = train_data, 
                    linear.output = FALSE, err.fct = "ce", rep = 200,
                    likelihood = TRUE, hidden = 1)
...
assign_mat <- tibble(
  ...
  mutate(sample_size = sum(value)) %>%
  ungroup() %>%
  mutate(value = value/sample_size)

roc_df <- tibble(
  fpr = as.numeric(pref@x.values[[1]]),
  tpr = as.numeric(pref@y.values[[1]])
)

ggplot(assign_mat, aes(x = predicted_status, y = true_status)) +
  geom_tile(aes(fill = value), color = "grey", size = 0.5) +
  geom_text(aes(label = paste("N =", value*sample_size))) +
...
ggplot(roc_df, aes(fpr, tpr)) +
  geom_line() +
  geom_abline(aes(intercept = 0, slope = 1), col = "red", alpha = 0.5, linetype = 2) +
  labs(x = "False positive rate", y = "True positive rate") +
  ggtitle(paste("AUC = ", auc)) +
  theme_classic(base_size = 14) 

```

Compare to a model using just mean cases.
