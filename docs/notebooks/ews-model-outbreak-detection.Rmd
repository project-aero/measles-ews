...
for(do_city in all_cities){
  ...
  all_states <- filter_states(M1, mles, num_particles = 500)
...
run_ews_analysis <- function(df, test_year, bw = 15, ews_to_ignore){
  ...
  ews_trends <- get_trends(x = log(cases+1), bandwidth = bw,
...
combined_trends <- ews_trends %>%
  ...
  spread(model, proportion) %>%
  mutate(ews = ifelse(is.na(ews) == TRUE, 0, ews)) %>%
  gather("model", "proportion", ews:parametric)
...
```{r prop-diffs}
diffed_trends <- combined_trends %>%
  mutate(proportion = ifelse(true_outbreak_year == FALSE, 
                                   1-proportion, proportion)) %>%
  spread(key = model, value = proportion) %>%
  mutate(prop_diff = parametric - ews)

ggplot(diffed_trends, aes(x = lead_time, y = as.factor(year))) +
  geom_tile(aes(fill = prop_diff)) +
  facet_grid(city~true_outbreak_year) +
...
If this sounds promising to the group, I think the next step is to simulate many, many 10 year time series and then divide the years into outbreak and non-outbreak groups.
I'll do enough replicates to get, say, 50 outbreak and 50 non-outbreak years (sample these from ~200 `pomp::simulate` replicates).
Then do the same analysis as above for all replicates within a group and summarize results.