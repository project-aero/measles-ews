...
new_data <- pomp::simulate(M1, nsim = 1, params = unlist(mles), seed = 1243765,
...
combined_trends <- ews_trends %>%
  ...
  dplyr::select(model, year, proportion, lead_time, true_outbreak_year) %>%
  bind_rows(model_trends %>%
              mutate(proportion = n/max(n),
                     model = "parametric") %>%
              dplyr::select(model, year, proportion, 
                            lead_time, true_outbreak_year))

ggplot(combined_trends, aes(x = lead_time, y = proportion)) +
  geom_line(aes(linetype = model, color = true_outbreak_year)) +
  facet_wrap(~year) +
  scale_x_reverse() +
  scale_color_discrete(name = "true classification", labels = c("no outbreak", "outbreak")) +
  labs(x = "lead time (weeks)",
       y = "proportion of ews increasing\nor\nproportion of simulations predicting outbreak")
```
