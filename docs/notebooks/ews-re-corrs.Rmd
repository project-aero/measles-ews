...
measles_data <- measles_data %>%
  mutate(month = lubridate::month(date),
         season_year = ifelse(month < 8, year, year+1))
...
ggplot(filter(yearly_stats_ews, region == "Niamey (City)" & 
                ews == "variance" &
                month > 7,
                season_year > 1995), 
  ...
  facet_wrap(~season_year, scales = "free")
...
# Start time in September

```{r}
yearly_stats_ews <- yearly_stats_ews %>%
  mutate(month = lubridate::month(date),
         season_year = ifelse(month < 9, year, year+1))
```
