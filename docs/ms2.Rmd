...
We focus on two scenarios: the re-emergence of measles following a large outbreak, a situation typical of measles dynamics in sub-Saharan Africa, and the elimination of measles by a vaccination campaign.
...
## Data
We used weekly measles case report data from four Nigerien cities: Agadez, Maradi, Niamey, and Zinder (Fig. \ref{data}A).
The data were collected over an 11 year period from 1995-2005 (Fig. \ref{data}B).
...
The fitted models adequately reproduce observed dynamics (Fig. \ref{data}B), with in-sample $R^2$s from one-step-ahead predictions ranging from 0.54 for Agadez to 0.89 for Maradi (Fig. \ref{scatters}A).
...
In fact, the high AUC values for Niamey result from exceptionally poor performance: they are all higher in the null interval rather than the test interval, contrary to theoretical expectations.
...
All three metrics decreased as $R_E(t)$ approached the critical transition (Fig. Sx).
...
We found similar results when calculating EWS over a moving window of 35 weeks in the null and test intervals (SI text, Fig. Sx).
