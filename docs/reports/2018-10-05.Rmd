...
ews_state_corrs <- ews_state_corrs %>%
  mutate(
    ews_abbs = NA,
...
data <- ews_state_corrs %>%
  ungroup() %>%
  filter(state == "effective_r_seasonal") %>%
  dplyr::select(ews_abbs, spearman_value) %>%
  spread(key = ews_abbs, value = spearman_value)

data <- rbind(rbind(rep(1,10) , rep(-1,10), data))
radarchart(data)
 
# Custom the radarChart !
radarchart( data  , axistype=1 , seg = 2,centerzero = TRUE,pty = 32,
 
    #custom polygon
    pcol=rgb(0.2,0.5,0.5,0.9) , plwd=1 , 
 
    #custom the grid
    cglcol="grey", cglty=1, axislabcol="grey", caxislabels=c(-1,0,1), cglwd=0.8,
...
# ggplot(data = NULL, aes(x  = ews_abbs, y = spearman_value) ) +  theme_minimal() +
#   geom_polygon(data = df2,
#                aes(y = ymin,
#                    group = state),
#                linetype = 2, fill = "grey") +
#   geom_polygon(data = df2,
#                aes(y = ymax,
#                    group = state),
#                linetype = 2, fill = "white") +
#   geom_polygon(data  = ews_state_corrs, aes(group = state, color = state),
#                fill  = NA,   show.legend = FALSE,size = 0.4) +
...
#   scale_color_manual(values = rep("dodgerblue", 3))+
#   facet_wrap(~state) +
#   coord_radar()

ttest <- function(r = 0.082, n = 572){
  r*sqrt((n-2)/(1-r^2))
}
...
```{r prelim-ews-trans, echo = FALSE, eval=FALSE}
ggplot(filtered_states, aes(x = med)) +
  geom_histogram() +
  facet_wrap(~state, scales = "free") +
  scale_x_log10()

seasonal_r <- filtered_states %>%
  filter(state == "effective_r_seasonal") %>%
  pull(med)

diptest::dip.test(log(seasonal_r))
```
