...
last_iter <- mif_traces %>%
  ...
  mutate(
    ...
    close_to_ll = "grey",
    close_to_ll = ifelse(delta_loglik < 5.00001, "red", close_to_ll)
  ) %>%
  filter(delta_loglik < 100) %>%
  dplyr::select(-do_grid, -iteration, -nfail, -delta_loglik,
                -b1, -b2, -b3, -b4, -b5, -b6)

# pairs(last_iter[,1:(ncol(last_iter)-1)], col = last_iter$close_to_ll, pch = 1, cex = 1)

vars <- colnames(last_iter)[1:(ncol(last_iter)-1)]
par(mfrow = c(length(vars), length(vars)), mar=c(1,1,1,1))
for(i in 1:length(vars)){
  for(j in 1:length(vars)){
    tmp1 <- last_iter[,c(i,ncol(last_iter))]
    tmp2 <- last_iter[,c(j,ncol(last_iter))]
    x1 <- as.numeric(unlist(tmp1[which(tmp1$close_to_ll=="grey"),1]))
    y1 <- as.numeric(unlist(tmp2[which(tmp2$close_to_ll=="grey"),1]))
    x2 <- as.numeric(unlist(tmp1[which(tmp1$close_to_ll=="red"),1]))
    y2 <- as.numeric(unlist(tmp2[which(tmp2$close_to_ll=="red"),1]))
    
    if(i == j){
      plot(x1, y1, xlab = vars[i], ylab = vars[j], col = "white", pch = 19, cex = 0.5)
      points(x2, y2, col = "white", pch = 19, cex = 0.5)
      text(x = mean(range(c(x1,x2)), na.rm = T), y = mean(range(c(y1,y2)), na.rm = T), labels = vars[i])
    }
    if(i != j){
      plot(x1, y1, xlab = vars[i], ylab = vars[j], col = "grey", pch = 19, cex = 0.5)
...
## ZINDER (population 322,935)

### Traceplots of the iterated filtering paths for the parameter sets with the 200 highest log likelihoods
```{r traces-zd, echo = FALSE, warning=FALSE, message=FALSE}
DO_CITY <- "Zinder"
...
```{r lik-slice-zd, warning=FALSE, message=FALSE}
last_iter <- mif_traces %>%
  filter(iteration == 100) %>%
  mutate(
    delta_loglik = abs(loglik - max(loglik)),
...
### Parameter estimates with the highest log likelihood
Note that the model was fit with a time-step of 1 day (`delta.t = 1/365`).
```{r mles-zd, echo = FALSE, warning=FALSE, message=FALSE}
mles <- mif_finals %>%
  filter(loglik == max(loglik, na.rm = TRUE)) %>%
  dplyr::select(-do_grid) %>%
  dplyr::select(-b1, -b2, -b3, -b4, -b5, -b6)
...
### Model simulations at the (current) MLEs
```{r sims-zd, echo = FALSE, warning=FALSE, message=FALSE}
mles <- mif_finals %>%
  filter(loglik == max(loglik, na.rm = TRUE)) %>%
  dplyr::select(-do_grid, -loglik, -loglik_se)
...
```{r filtering-dists-zd, warning=FALSE, message=FALSE}
filtered_states <- readRDS("../../results/filtered-states-Zinder.RDS") %>%
  unnest()
...
ll_tvary <- read.csv("../../results/time-vary-beta-loglik-Zinder.csv") %>%
  dplyr::select(-X) %>%
  spread(var, value) %>%
  mutate(
    model = "transmission random walk"
...
### Correlations with EWS
```{r ews-zd, warning=FALSE, message=FALSE}
# Early warning signals
ews_results <- readRDS("../../results/ews-niger-cities.RDS") %>%
  filter(str_detect(city, paste0("^", DO_CITY))) %>%  # city starts (^) with DO_CITY
  rename(ews_value = value)
