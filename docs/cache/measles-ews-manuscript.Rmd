...
```{r mles, warning=FALSE, message=FALSE, results='asis', echo=FALSE, eval = FALSE}
library(tidyverse)
library(dplyr)
library(xtable)
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.5\textwidth]{../figures/var-ews-example.pdf}
\caption{\textbf{Boxplots of the variance in null (far from $\bm{R_E = 1}$) and test (close to $\bm{R_E = 1}$) intervals}. As expected by theory, the variance decreases on the approach to elimination and the variance increases on the approach to emergence. Greater separation between the null and test distributions leads to higher AUC values in Fig. 3. Variance is log transformed for visual purposes.}
\label{fig:var-ews}
\end{figure}
