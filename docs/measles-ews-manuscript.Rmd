...
In the approach to re-emergence, the EWS generally perform as expected, with Niamey being an exception (Fig. \ref{emerge-aucs}).
...
ggplot(filter(one_sim, time < 11.5), aes(x = time, y = reports)) +
  ...
  annotate(geom = "text", x = 1.75, y = 250, size = 3,
           label = "Null window", fontface = "bold") +
  annotate(geom = "text", x = 5.25, y = 250, size = 3,
           label = "Test window", fontface = "bold") +
...
```{r elimination, fig.width=8.5, fig.height=5}
