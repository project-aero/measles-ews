...
The generic indicators performed better in scenarios of re-emergence, and moreover these indicators behaved as expected based on a generic model of fluctuations around an equilibrium with gradually declining stability --- the prototypical model of critical slowing down.
This is because the autocorrelation and other EWS increased on the approach to $R_E = 1$ (figure \ref{emerge-aucs}*c*).
If, alternatively, the variance had increased but the autocorrelation had not, an alternative generic model would be an equilibrium with fixed stability but subject to increasing intensity of perturbations.
...
And since our simulations were fit to measles data and had high one-step-ahead predictive accuracy, it seems reasonable to suppose that the prototypical model of critical slowing down are supported by read data and can provide predictive value.
...
The autocorrelation was not a strong indicator of the transition to elimination (figure \ref{elim-aucs}*c*), suggesting weaker evidence for CSD.
