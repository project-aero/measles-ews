...
Second, CSD can anticipate disease re-emergence and elimination?
...
These data are ideal for testing theory on CSD in disease dynamics because each city has a different population size (spanning means of about 15 to 75 thousand during this time period), different dynamics in terms of outbreak sizes (maximum weekly incidence from 60 to 1845 cases) and length of inter-epidemic periods (2-5 years), and has different amounts of demographic stochasticity due to differences in population size.
...
To simulate elimination, we simulated a vaccine campaign in which the vaccination probability linearly increased over time to eventually reach 100\%, i.e. eradication (figure S3).
...
We define the test interval as the window of time between the start of the vaccination campaign (year 50) and the time at which the vaccination probability reached the vaccination threshold of $1 - 1/\max(\mathcal{R}_0 (t))$ (figure \ref{elim-aucs}*a*);
...
We define the null interval as the window of time that ends at the start of the vaccination campaign (end of year 49) and starts at a time that results in an interval equal in length to the test interval (figure \ref{elim-aucs}*a*).
...
Stochastic simulations of the models displayed dynamics typical of each city (figure S2), including the decline in seasonality amplitude as population size decreases (figure \ref{scatters}*b*) [@Ferrari2008].
