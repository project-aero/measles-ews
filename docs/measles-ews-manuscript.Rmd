...
{\LARGE
\textbf\newline{Critial slowing down anticipates emergence and elimination of measles in Niger} 
}
...
Whether EWS anticipate critical transitions in real disease time series remains unknown, and is a critical knowledge gap that must be filled before EWS can confidently be deployed.

Here, we use empirically-based model simulations of measles dynamics to test whether EWS anticipate critical transitions in realistic disease dynamics.
Our focus is on the re-emergence of measles following a large outbreak, a situation typical of measles dynamics in sub-Saharan Africa.
We seek to answer two related questions.
First, can EWS distinguish between time series of disease incidence when the underlying dynamics are far from and near to a critical transition?
If so, then EWS can anticipate disease re-emergence and large outbreaks.
Second, how does the distance to and the rate of approaching the epidemic threshold impact the performance of EWS?
It may be that some EWS perform well when disease re-emergence is slow, while others perform well when the epidemic threshold is rapidly reached.

To answer these questions, we fit mechanistic models of disease transmission to time series of measles incidence in four Nigerien cities: Agadez, Maradi, Niamey, and Zinder.
We then use the fitted models to perform model experiments designed to test the performance of EWS at anticipating critical transitions after susceptible depletion.
Our results confirm theoretical expectations about several EWS.
In particular, we show that several EWS anticipate critical transitions in realistic scenarios, and they do so using much shorter time series than used in theoretical studies.
However, our study also highlights the limitations of EWS in situations where disease re-emergence occurs rapidly.
...
\caption{(A) Locations and population sizes (in parantheses) of our four focal cities in Niger. (B) Time series of reported cases (yellow solid lines) and the 95\% prediction intervals (black ribbons) for one-step-ahead forecasts from our fitted SEIR models for each city.}
\label{fig:data}
\end{figure}
...
That is, we do not use a moving window and if, for example, the length of the interval is 520 weeks (10 years), then $b = 0.5\times 520 = 260$.
...
The fitted models adequately reproduce observed dynamics (Figure 1), with in-sample $R^2$s from one-step-ahead predictions ranging from 0.54 for Agadez to 0.89 for Maradi (Figure \ref{fig:scatters}).
...
The EWS generally perform as expected, with Niamey being an exception (Figure \ref{fig:aucs}).
...
For the other cities, variance, mean, index of dispersion, decay time, autocovariance, and autocorrelation all perform equally well (Figure \ref{fig:aucs}).
