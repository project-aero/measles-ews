...
the_map <- ggplot(region_boundaries)+
  ...
  annotate("text", x = 12, y = 20, label = "Niger", fontface = 3, size = 5) +
...
We calculated AUC as: $\text{AUC} = \left[r_{\text{test}} - \left(n_{\text{test}}+1\right)/2\right] / \left(n_{\text{test}}n_{\text{null}}\right)$ where $r_{\text{test}}$ is the sum of the ranks of test set EWS statistics in a combined set of null and test statistics (lower numbers have lower ranks), $n_{\text{test}}$ is the number of test of statistics and $n_{\text{null}}$ is the number of null statistics.
...
The fitted models also had higher negative log likelihoods than two benchmarking models: a negative binomial sampling model assuming identical and idependently distributed observations and a seasonal moving average model (SI Table Sx).
...
On the approach to re-emergence the EWS generally perform as expected by theory.
Most EWS increased as the critical transition is approached, resulting in positive AUC values near 1 (Fig. \ref{emerge-aucs}).
...
Variance, mean, index of dispersion, decay time, autocovariance, and autocorrelation all perform equally well at predicting re-emergence (Fig. \ref{emerge-aucs}C).
...
Thus, re-emergence may prove difficult to anticipate in "fast" transmission systems, as demonstrated theoretically by @Brett2017 and seen here when susceptible depletion was relatively small (Fig. \ref{emerge-aucs}C).
...
ggplot(filter(one_sim, time < 11.5 & time > 0.6), aes(x = time, y = reports)) +
  ...
  annotate(geom = "text", x = 2.35, y = 250, size = 3,
           label = "Null interval", fontface = "bold") +
  annotate(geom = "text", x = 6, y = 250, size = 3,
           label = "Test interval", fontface = "bold") +
  annotate(geom = "text", x = 2.35, y = 220, size = 3,
           label = paste("Variance =", 
                         filter(variance_one_sim, interval == "null") %>%
                           pull(variance))) +
  annotate(geom = "text", x = 6, y = 220, size = 3,
           label = paste("Variance =", 
                         filter(variance_one_sim, interval == "test") %>%
                           pull(variance))) +
  scale_alpha_manual(values = c(0.4, 1)) +
  guides(alpha = FALSE) +
  annotate(geom = "text", x = 7.3, y = 125, label = "critical\nyear", 
           size = 3, color = "dodgerblue4") +
  labs(x = "Simulation time (year)", y = "Reported cases") +
  theme_classic(base_size = 11) -> cases_series
...
ggplot(variance_all_sim, aes(x = log(variance + 1), fill = interval)) +
  ...
  annotate(geom = "text", x = 3, y = 1.2, size = 3,
...
row2 <- cowplot::plot_grid(aucs, labels = "C")
row1 <- cowplot::plot_grid(cases_series, dist_plot, scale = 0.9, labels = "AUTO")
final <- cowplot::plot_grid(row1, row2, ncol = 1, align = "v")
print(final)
```


The EWS did not perform as well when anticipating elimination, relative to emergence (Fig. \ref{elim-aucs}).
...
ggplot() +
  ...
  geom_line(data = ews_data_example, aes(x = time, y = reports),
            color = "tan", size = 0.3) +
  annotate(geom = "text", x = 28, y = 1450, size = 3,
           label = "Null interval", fontface = "bold") +
  annotate(geom = "text", x = 70, y = 1450, size = 3,
           label = "Test interval", fontface = "bold") +
  annotate(geom = "text", x = 36, y = 900, 
           label = "start of\nvaccination campaign", 
           size = 2, color = "grey25") +
  annotate(geom = "text", x = 79, y = 900, 
           label = "coverage reaches\nherd immunity",
           size = 2, color = "dodgerblue4") +
  annotate(geom = "text", x = 28, y = 1300, size = 3,
           label = paste("Variance =", 
                         filter(variance_example, half == "first") %>% 
                           pull(variance))) +
  annotate(geom = "text", x = 70, y = 1300, size = 3,
           label = paste("Variance =", 
...
ggplot(data = all_variances, aes(x = log(variance + 1), fill = interval)) +
  geom_density(size = 0.5, alpha = 0.65, color = "white") +
  annotate(geom = "text", x = 10, y = 0.4, size = 3,
           label = paste("Area under the curve:", var_auc)) +
  scale_fill_manual(values = mycols, name = "Interval", 
                    labels = c("Null", "Test")) +
  labs(x = "Log(variance + 1)", y = "Density") +
...
row2 <- cowplot::plot_grid(aucs, labels = "C")
row1 <- cowplot::plot_grid(cases_series, dist_plot, 
                           scale = 0.9, labels = "AUTO")
final <- cowplot::plot_grid(row1, row2, ncol = 1, align = "v")
print(final)
```
...
<!-- A change in the variance of a dynamical system is one of the most well-studied early warning signals [@Scheffer2012], and it performed well here (Figs. \ref{emerge-aucs}, \ref{elim-aucs}). -->
...
A potential limitation of our findings is that the levels of susceptible depletion we modeled (Fig. \ref{emerge-aucs}C) might be lower than the levels that occur in reality.
...
For example, AUC values for emergence at the 0.5 level of susceptible depletion are already low for most cities (Fig. \ref{aucs}A).
...
```{r var-ews, fig.width=4, fig.height=4, fig.cap="Boxplots of the variance in null (far from $R_E$ = 1) and test (close to $R_E$ = 1) intervals. As expected by theory, the variance decreases on the approach to elimination and the variance increases on the approach to emergence. Greater separation between the null and test distributions leads to higher AUC values in Fig. 3. The plotted results from emergence simulations where the susceptible discounting factor is 1E-04 and from elimination simulations where the speed to herd immunity is 1.5E-05. Variance is log transformed for visual purposes. \\label{var-ex}", eval = FALSE}
do_metric <- "variance"
