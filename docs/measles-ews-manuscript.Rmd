...
These findings suggest that early warning signals could be operationalized to develop early warning systems of disease emergence, elimination, or outbreaks of endemic diseases.

However, operationalizing EWS, and deploying early warning systems based on them, faces many challenges.
...
That is, EWS might perform well for a given time series, but for reasons having nothing to do with critical slowing down.
...
This means our conclusions are not based on one-off events.
...
In addition to comparing model expectations to in-sample observed data, we also compared our fitted SEIR models to two benchmarking models: a negative binomial sampling model that assumes independent and identically distributed observations and a seasonal moving average model (SI text).
...
<!-- Comparing our fitted mechanistic models to non-mechanistic benchmarking models allows us to assess whether our fitted models are lending mechanistic insight and thus worthy of further inference. -->
...
We then simulated the model forward for forty years using mean birth rate for the entire country ($\mu = 0.05$) and setting the death rate equal to the birth rate ($\mu = \nu = 0.05$) to achieve a constant eqiulibrium total population size over the course of the simulation (total population size does vary, though, because of stochasticity in the model).
...
Variance, mean, index of dispersion, decay time, autocovariance, and autocorrelation all perform equally well at predicting re-emergence (figure \ref{emerge-aucs}*c*).
...
ggplot(filter(one_sim, time < 11.5 & time > 0.6), aes(x = time, y = reports)) +
  ...
  annotate(geom = "text", x = 2.35, y = 220, size = 3, family = "Times",
           label = paste("variance =", 
                         filter(variance_one_sim, interval == "null") %>%
                           pull(variance))) +
  annotate(geom = "text", x = 6, y = 220, size = 3, family = "Times",
           label = paste("variance =", 
...
As in the case of anticipating elimination, AUC values decreased as the speed of the vaccine campaign increased (figure \ref{elim-aucs}*c*).
...
For example, AUC values for emergence at the 0.5 level of susceptible depletion are already low for most cities (figure \ref{aucs}A).
