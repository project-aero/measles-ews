...
ggplot(variance_all_sim, aes(x = log(variance + 1), fill = interval)) +
  ...
  scale_fill_manual(values = c(ggthemes::ptol_pal()(2)), 
                     name = "Interval", labels = c("Null", "Test")) +
...
ggplot() +
  ...
  annotate(geom = "text", x = 28, y = 1350, size = 3,
           label = paste("Variance =", 
                         filter(variance_example, half == "first") %>% 
                           pull(variance))) +
  annotate(geom = "text", x = 70, y = 1350, size = 3,
           label = paste("Variance =", 
...
ggplot(data = all_variances, aes(x = log(variance + 1), fill = interval)) +
  geom_density(size = 0.5, alpha = 0.65, color = "white") +
  annotate(geom = "text", x = 4, y = 0.3, size = 3,
           label = paste("Area under the curve:", round(var_auc,2))) +
  scale_fill_manual(values = c(ggthemes::ptol_pal()(2)), 
                     name = "Interval", labels = c("Null", "Test")) +
  labs(x = "Log(variance + 1)", y = "Density") +
  theme_classic(base_size = 11) +
  theme(legend.position = c(0.2, 0.8)) -> dist_plot
