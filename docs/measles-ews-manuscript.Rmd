...
\noindent{}where $\beta_t$ is a time-varying rate of transmission, $\eta$ is a time-invariant rate from the exposed class to the infectious class, and $\gamma$ is a time-invariant recovery rate.
...
\comment{$\beta$ is not the minimum value of $\beta_t$ in the above equation unless all $q_i$ are positive, and I believe at least for Niamey some are estimated to be negative}
\noindent{} where $\beta$ is the baseline transmission rate over the season, and the term $\sum^6_{i=1} q_i \xi_{i_{t}}$ is a B-spline to model seasonality in transmission.
