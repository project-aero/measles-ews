...
We define the null interval as the window of time that ends at the start of the vaccination campaign (year 49) and starts at a time that results in an interval equal in length to the test interval (figure \ref{elim-aucs}*a*).
...
But a natural expectation is that the mean should decrease and the variance should decrease.
...
Maximum likelihood estimates and bootstrapped 95% confidence intervals for all parameters are in the electronic supplement (tables Sx-Sz).
...
We found similar results for the approach to elimination when calculating EWS over a moving window of 35 weeks in the null and test intervals (electronic supplement, figure S7).
