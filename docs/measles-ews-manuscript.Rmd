...
Because we assumed that total population size was known, we only explicitly modeled imports into the infected class that would influence the reported cases.
...
For example, if the total length of the time series from the beginning of the simulations to the time at which $\mathcal{R}_e(t)$ reached or exceeded 1 was six years, then the first window would be years 1 through 3, and the second window would be years 4 through 6. 
...
The null interval was defined as the window of time that ends at the beginning of the vaccination ramp up (end of year 49) and starts at a time that results in an interval equal in length to the test interval (figure \ref{elim-aucs}*a*).
For example, if the vaccination threshold for herd immunity was met in year 60 of the simulation, then the test interval was defined as years 50 (when vaccination ramp up began) through 60; the null interval would then be defined as years 39 through 49 to match the length of the test interval.
