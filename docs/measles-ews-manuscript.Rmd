...
Critical slowing down (CSD) can be measured by changes in the statistical properties of the system, often referred to as early warning signals (EWS), such as an increase in the variance and autocorrelation [@Carpenter2006; @Scheffer2009].
...
Such differences provide a natural gradient of extra-demographic and environmental stochasticity that may influence critical slowing down [@Hastings2010; @Dakos2012a; @ODea2018a; @ORegan2018].
...
The stochastic random variables are distributed as follows:
...
\noindent{} where $\beta$ is the minimum transmission rate over the season, and $\sum^6_{i=1} q_i \xi_{i_{t}}$ is a B-spline to fit seasonality in transmission.
...
Death was not included in the model because the rate of infection is much faster than the rate of death and because we use known population size at each time step.
Excluding deaths means we can avoid making further assumptions about demography.
Demographic stochasticity in births and imported infections wwere represented by time-specific Poisson random variates.
...
We estimated 14 parameters: six seasonal transmission parameters ($q_i$), minimum transmission rate ($\beta$), three initial conditions $\left(S_{(t=0)},E_{(t=0)},I_{(t=0)}\right)$, the importation rate ($\psi$), reporting fraction ($\rho$), a parameter accounting for process noise ($\sigma$), and a parameter accounting for measurement noise ($\tau$).
...
We fit these benchmark models to the observed data and then compared models using Akaike's Information Criterion (AIC) to account for differences in the number of model parameters [@Akaike1973].
...
Simulation details are in the electronic supplement.
...
First, averaging across all simulations for a city--susceptible-depletion combination, we found the simulation year in which $\mathcal{R}_e(t)$ reached or exceeded 1 and excluded years after that (electronic supplement).
...
We performed simulations for 100 years, starting with 50 years of at the baseline vaccine coverage reported for Niger of 70\%, $p_t = 0.7$ [@Ferrari2008].
...
We performed simulations across six vaccination "speeds" (the rate at which $p_t \rightarrow 1$; electronic supplement), simulating situations of slow and fast approaches to elimination.
As in re-emergence simulations, we set the birth rate equal to the death rate to achieve a constant equilibrium population size. 
...
$\max(\mathcal{R}_0(t))$ was determined for each city using the MLE parameters as: $\max(\mathcal{R}_0) \approx \left[\eta \max(\beta) \right]/ \left[\left(\eta+\nu\right)\left(\gamma+\nu\right)\right]$, where $\max(\beta)$ is the maximum value of $\beta_t$ over its seasonal variation.
The null interval was defined as the window of time that ends at the beginning of the vaccination remp up (end of year 49) and starts at a time that results in an interval equal in length to the test interval (figure \ref{elim-aucs}*a*).
...
A small discount factor corresponds to a large level of susceptible depletion.
...
Thus, our work also does not represent how EWS would be operationalized in online mode.
...
But there will liekly always be a role for expert judgment.
