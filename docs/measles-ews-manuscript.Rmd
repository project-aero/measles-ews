...
On the approach to re-emergence, the EWS generally perform as expected, with Niamey being an exception (Fig. \ref{emerge-aucs}).
...
For the other cities, variance, mean, index of dispersion, decay time, autocovariance, and autocorrelation all perform equally well at predicting re-emergence (Fig. \ref{emerge-aucs}C).
...
Moreover, as the time to reach $R_E(t)=1$ decreases, the chance of a bifurcation delay increases due to the changing equilibrium of $R_0$ combined with demographic stochasticity [@Dibble2016; @Brett2017].
Thus, re-emergence may prove difficult to anticipate in "fast" transmission systems, as demonstrated theoretically by @Brett2017 and seen empirically here in the Niamey dynamics (Figs. \ref{scatters}B, \ref{emerge-aucs}C).
...
# Case time series
ggplot(filter(one_sim, time < 11.5), aes(x = time, y = reports)) +
  ...
  annotate(geom = "text", x = 1.75, y = 250, size = 3,
           label = "Null interval", fontface = "bold") +
  annotate(geom = "text", x = 5.25, y = 250, size = 3,
           label = "Test interval", fontface = "bold") +
  annotate(geom = "text", x = 1.75, y = 220, size = 3,
           label = paste("Variance =", 
                         filter(variance_one_sim, interval == "null") %>%
                           pull(variance))) +
  annotate(geom = "text", x = 5.25, y = 220, size = 3,
           label = paste("Variance =", 
                         filter(variance_one_sim, interval == "test") %>%
                           pull(variance))) +
  scale_alpha_manual(values = c(0.4, 1)) +
  guides(alpha = FALSE) +
  annotate(geom = "text", x = 6.3, y = 125, label = "critical\nyear", 
...
A potential limitation of our findings is that the levels of susceptible depletion we modeled (Fig. \ref{aucs}A) might be lower than the levels that occur in reality.
...
Early warning signals, though powerful and now accompanied with robust empirical support, will likely be just one part of a decision-support toolkit.
