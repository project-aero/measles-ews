...
\new{We also used data on district population sizes and national-level birth rates (figure S1).}
...
\noindent{}\new{Event zero is the event that an individual stays in its initial compartment and does not contribute to the vector $\textbf{n}_t$.}
...
\new{Doing so allows us to gain some intuition as to whether accounting for mechanism (i.e., transmission dynamics) improves model fit and inference.}
...
\new{A recent study using a different method for detectin critical transitions found that elimination was easier to detect than re-emergence, contrary to our findings} [@parag2022].
\new{This suggests different approaches could be considered for different applications.}

\new{Kurtosis and skewness did not perform well (AUC far from 0 and 1).}
\new{This may be because higher moment based indicators may need longer time series and higher amounts of stochasticity to detect critical transitions.}
\new{Thus, while kurtosis and skewness were not strong indicators of the transition to re-emergence in our system, they may still prove useful in other systems.}
...
\new{We also found that EWS in smaller cities performed worse relative to larger cities at predicting elimination (figure \ref{elim-aucs}\textbf{c}).}
\new{Our hypothesis is that stochastic effects are stronger in smaller populations, which introduces more variation into our EWS and makes trends more difficult to identify.}
\new{Spatial replication across small populations, which should reduce stochasticity, could prove important for developing practical EWS.}
...
\new{Another potential limitation is that the population of Nigeria has increased substantially since the end of the time series’ we analyzed in 2005. This presents a limitation to our work because we cannot definitively say whether our results are robust to such large changes in population size. However, our analysis across cities with different population sizes suggests that EWS would remain a viable tool. Elimination may be easier to detect, as demonstrated by our result that EWS performed better in larger cities.}
...
\new{Determining the optimal moving window width is non-trivial} [@Brett2020] \new{and is beyond the scope of this article.}
...
\new{Providing this link from theory to application through empirically-based models represents an important step toward operationalising EWS.}
\new{Further applied work on optimal moving window widths and the lead-time of different EWS appears to be worth the effort given our encouraging results at the theoretical-applied divide.}
\new{This is especially true because fitting even relatively simple models of disease tranmission for a well-known disease such as measles is difficult.}
\new{The ability to rely on model-free indicators of critical transitions therefore remains an important endeavor, especially for emerging pathogens.}
