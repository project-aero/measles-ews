...
Second, how does the distance to and the rate of approaching the threshold impact the anticipatory accuracy of CSD?
...
\noindent{}where $\Delta t$ is a time step of $\text{year} / 365$, and $\textbf{n}_t$ are random variables representing the number of individuals transitioning into or out of each class at each timestep $t \rightarrow t+\Delta t$.
...
\begin{align}
n_{S,t} &\sim \text{Poisson}(\mu_t (1 - p) N_t \times \Delta t) \\
