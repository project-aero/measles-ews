...
cases_series <- ggplot(filter(one_sim, time < 11.5), aes(x = time, y = reports)) +
  ...
  annotate(geom = "text", x = 1.75, y = 250, 
           label = "Null window", fontface = "bold") +
  annotate(geom = "text", x = 5.25, y = 250, 
           label = "Test window", fontface = "bold") +
  annotate(geom = "text", x = 1.75, y = 220, 
           label = paste("Variance =", 
                         filter(variance_one_sim, interval == "null") %>%
                           pull(variance))) +
  annotate(geom = "text", x = 5.25, y = 220, 
           label = paste("Variance =", 
...
ggplot(variance_all_sim, aes(x = log(variance + 1), fill = interval)) +
  geom_density(size = 1, alpha = 0.65, color = "white") +
