...
for(do_city in unique(measles$region)){
  ...
  if (do_city == "Niamey (City)") narma <- list(arma_fit, arma_nll, log_y)
...
We used a daily time step of $dt = \text{year} / 365$.
...
Table: List of candidate early warning signals and their estimating equations. See @Brett2018 for details. \label{tab:ews}
...
```{r ews-hists, message=FALSE, warning=TRUE, echo = FALSE, fig.cap="Histograms of EWS from emergence simulations where the susceptible discount factor was 1e-04.", fig.height=5}
...
Essentially, this metric tells us the discounting factor that would need to be applied to *S* at its peak in year *t - 1* to match the reduction in $S$ caused by the outbreak of the size in year *t*.
