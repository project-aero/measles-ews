...
\begin{align}
n_{0S,t} &\sim \text{Poisson}((1-p)\mu N \times \Delta t) \\
...
n_{0R,t} &\sim \text{Poisson}(p \mu N \times \Delta t) \\
...
For re-emergence, we defined the year of the critical transition as the year just after the effective reproduction number ($\mathcal{R}_e$) reaches or exceeds the critical value of 1.  
For example, if $\mathcal{R}_e$ reaches or exceeds 1 at some point during the fifth year of the simulation, then the critical transition year is defined as the sixth year of the simulation. 
...
We took a conservative approach for calculating the vaccination threshold by using the maximum value of $\mathcal{R}_0(t)$ out of the range of values in its seasonal variation. That is, we set $p = 1 - 1/\text{max}(\mathcal{R}_0(t))$.
