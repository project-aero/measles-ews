...
The second benchmarking model is a seasonal autoregressive moving average model (an ARIMA (2,0,2)(1,0,1)\textsubscript{52} model) that can account for data dependencies and annual periodicity.
...
The NBM model has 2 parameters, the SARIMA model has 8 parameters, and the SEIR model has 14 parameters.
...
\begin{align}
n_{0S,t} &\sim \text{Poisson}((1-p)\mu N \times dt) \\
(n_{SE,t}, n_{S0,t}) &\sim \text{EulerMultinomial}(S_{t}, (\beta_t I_t / N, \nu), dt) \\
(n_{EI,t}, n_{E0,t}) &\sim \text{EulerMultinomial}(E_{t}, (\eta, \nu), dt)  \label{eq:emulti} \\
(n_{IR,t}, n_{I0,t}) &\sim \text{EulerMultinomial}(I_{t}, (\gamma, \nu), dt) \\
n_{I0,t} &\sim \text{Poisson}(\psi \times dt) \\
n_{0R,t} &\sim \text{Poisson}(p \mu N \times dt) \\
n_{R0,t} &\sim \text{EulerMultinomial}(R_t, (\nu), dt),
\end{align}
\noindent{}where *p* is the vaccination coverage (set at $p = 0.7$), $\mu$ is the constant birth rate (set at $\mu = 0.05$), $N$ is the equilibrium population size, $\sim \text{EulerMultinomial}(T, r_i, dt)$ specifies that the variables on the left follow an Euler Multinomial distribution for $T$ individuals with hazard rates $r_i$ and step size $dt$, and $\psi$ is the rate of imported infections (estimated by the model). 
The Euler Multinomial distribution corresponds to a multinomial distribution where the event probabilities $p_i$ are determined by the Euler time step $dt$ and the hazard rates $r_i$, according to 
\begin{align}
p_0 &= \exp( -\sum_i r_i dt), \\
p_i &= (1 - p_0) r_i / (\sum_i r_i)\qquad \text{for}\qquad i > 0.
\end{align}
Event zero is the event that an individual stays in its initial compartment and does not contribute to the counts
respectively.
...
\noindent{}where $\beta_t$ is a time-varying rate of transmission, $\eta$ is a time-invariant rate from the exposed class to the infectious class, $\gamma$ is a time-invariant recovery rate, $\nu$ is the constant death rate, and $N$ is an equilibrium population size.
We set $1/\eta = 8$ days, $1/\gamma = 5$ days, and $\nu = 0.05$. We used a daily time step of $dt = \text{year} / 365$.
...
$$
R_E(t) \approx \frac{S_t}{N_t} \frac{\eta \beta_t}{(\eta + \nu)(\gamma + \nu)}.
$$
...
When determining the critical year, we eliminate the variation in $R_E$ that arises from the gamma white noise factor and thus calculate the expected value of $R_E(t)$ over the set of all simulation replicates at time $t$.
From the full window ranging from the beginning of the simulation to the end of the critical year, we defined the null interval as the first half of the window (far from $R_E = 1$) and the test interval as the second half of the full window (near $R_E = 1$).
Figure \ref{emerge-sim} shows a typical example.

```{r emerge-windows, fig.width=6, fig.cap="A typical emergence simulation for Maradi where the initial number of susceptibles was scaled by 1e-04. (*a*) The simulated trajectory of $R_E$ and the year in which $R_E$ first reaches the critical value of 1 (denoted by dashed blue line). (*b*) The simulated trajectory of the number of cases. Note that the x-axis has been reduced relative to the top panel. The two vertical blue lines indicate the start (left-most line) and end (line for critical year) of the full window. The black line demarcates the division between the equal-length null and test intervals. \\label{emerge-sim}"}
...
Because our transmission function is seasonal, we first aproximated time-specific $R_0$ as: $R_0(t) = \frac{\eta \beta_t}{(\eta+\nu)(\gamma+\nu)}$, where $1/\eta$ is the incubation period, $1/\gamma$ is the infectious period, $\beta_t$ is the time-specific rate of transmission, and $\nu$ is the death rate. Only $\beta_t$ is estimated by our model. We set $1/\eta$ = 8 days, $1/\gamma$ = 5 days, and $\nu$ = 0.05 per year.
Figure 2 in the main text shows our estimates of $R_0$ over the course of a year using this approximation.
We took a conservative approach for calculating the vaccination threshold by using the maximum value of $R_0(t)$ out of the range of values in its seasonal variation. That is, we set $p = 1 - 1/\text{max}(R_0(t))$.
...
So, we define the test interval as the times between the beginning of year 50 and the year at which vaccination coverage is equal to *p*.
We then defined the null interval as a window with length equal to the test interval and ending at end of year 49.
Figure \ref{elimin-sim} shows a typical example.

```{r elim-ex, fig.width=6, fig.cap="A typical elimination simulation for Maradi where the rate of change in vaccination coverage is 1.5e-05 per day. The null and test intervals are defined based on the time at which the vaccination campaign begins (year 50, black line) and the year at which vaccination coverage reaches the vaccination threshold for herd immunity (right-most dashed blue line). The beginning of the null interval is determined by the length of the series from time 50 to the time of herd immunity: the null interval ends at time 50 (when vaccination campaign begins) and starts at whatever time results in a series that is equal in length to the test interval. \\label{elimin-sim}"}
...
Table: List of candidate early warning signals and their estimating equations. See @Brett2018 for details.
...
```{r ews-hists, message=FALSE, warning=TRUE, echo = FALSE, fig.cap="Histograms of EWS from emergence simulations where the susceptible discout factor was 1e-04.", fig.height=5}
...
```{r ews-hists2, message=FALSE, warning=TRUE, echo = FALSE, fig.cap="Histograms of EWS from elimination simulations where the rate of change in vaccination coverage equals 1.5e-05 per day.", fig.height=5}
...
read_csv("../results/emergence-bandwidths-colnames.csv",
  ...
  geom_line() +
  geom_point(size = 3) +
  labs(x = "Susceptible discount factor", 
...
read_csv("../results/elimination-bandwidths-colnames.csv",
  ...
  geom_line() +
  geom_point(size = 3) +
  labs(x = "Rate of increase in vaccination coverage (per day)", 
       y = "Number of weeks in interval",
...
eliminate_plot <- ggplot() +
  ...
  scale_fill_viridis(limits = c(0,1), direction = -1, option = "E", name = "AUC") +
  facet_wrap(~city, nrow = 1) +
  labs(x = expression(paste("Rate of change in vaccine coverage (", phantom()%*%phantom(), 10^{-4}, phantom(0), d^{-1}, ")")), y = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(panel.spacing = unit(1, "lines"),
        plot.title = element_text(size = 11, face = "bold")) +
