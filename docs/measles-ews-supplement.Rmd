...
(n_{SE,t}, n_{S0,t}) &\sim \text{Mult}(S_{t}, (\lambda_{SE,t}, \lambda_{d,t})) \\
(n_{EI,t}, n_{E0,t}) &\sim \text{Mult}(E_{t}, (\lambda_{EI,t}, \lambda_{d,t})) \\
(n_{IR,t}, n_{I0,t}) &\sim \text{Mult}(I_{t}, (\lambda_{IR,t}, \lambda_{d,t})) \\
n_{I0,t} &\sim \text{Poisson}(\psi \times dt) \\
n_{0R,t} &\sim \text{Poisson}(p\mu N_t \times dt) \\
n_{R0,t} &\sim \text{Binomial}(R_t, \lambda_{d,t}),
\end{align}

\noindent{}where *p* is the vaccination coverage (set at $p = 0.7$), $\mu$ is the constant birth rate (set at $\mu = 0.05$), $\psi$ is the rate of imported infections (estimated by the model), and $\lambda_d$, $\lambda_{SE}$, $\lambda_{EI}$, and $\lambda_{IR}$ are the probabilities of death, exposure, becoming infectious, and recovery, respectively.
...
```{r ews-hists, message=FALSE, warning=TRUE, echo = FALSE, fig.cap="Histograms of EWS from emergence simulations where susceptible discout factor was 1e-04.", fig.height=5}
