...
model{
  ...
  # Parameter model
  for(t in 1:(ntimes-1)){
    beta[t] = exp(gamma0) * (1 + season[s[t]])
  }

  # for(t in 2:ntimes){
  #   epsilon[t] ~ dnorm(0, 1)
  #   # gamma[t] = gamma0 + t * log(1+r) + peps*epsilon[t]
  #   gamma[t] = gamma0 + peps*epsilon[t]
  # }
  ...
  # epsilon[1] ~ dnorm(0, 1)
  # gamma[1] = gamma0 + 1 * log(1+r) + peps*epsilon[1]
  gamma0 ~ dunif(-10.9, -10.7)
...
update(model, n.iter = 200000)

vars_to_collect <- c(
      "Iobs", "I", "S", "gamma", "beta", "rho", "lambda", 
      "psi", "alpha", "eta", "r", "sigma_gamma", "S0", "I0", 
      "gamma0", "pvalue_fit", "pvalue_mean", "pvalue_cv", "fit", "fitnew", "m", "beta_season"
    )
...
fitdf <- mcmcdf[, grep("fit", colnames(mcmcdf))]
