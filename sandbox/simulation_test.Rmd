...
```{r sim-function}
simulate_measles <- function(S0, I0, b, psi, rho, gamma0, r, nu, theta, sigma, dispersion, years){
  ...
  vareps <- rnorm(timesteps, 0, sigma)  # generate errors outside of loop
  ...
  for(t in 1:(timesteps-1)){
    beta[t] <- gamma0 + t * log(1-r) + vareps[t]
...
# Call the function
sim_out <- simulate_measles(S0, I0, b, psi, rho, gamma0, r, nu, theta, sigma, dispersion, years)
...
static_params <- time_invariants %>%
  mutate(
    real_value = c(gamma0, I0, psi, r, rho, S0, sigma)
  )
