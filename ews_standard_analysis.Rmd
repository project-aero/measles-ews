...
num_pred_years <- length(unique(tannout_data$year))
newdata <- tibble(
  week_num = seq(1, nrow(tannout_data), length.out = nrow(tannout_data)/num_pred_years*12),
  month = rep(1:12, times = nrow(tannout_data)/num_pred_years)
)
preds <- predict(mod$gam, newdata = newdata, type = "response", se.fit = TRUE)
...
ggplot(preds_tbl, aes(x = date)) +
  # geom_ribbon(aes(ymin = lower_prediction, ymax = upper_prediction)) +
  geom_point(aes(y = observation), col = "grey25", size = 1) +
  geom_line(aes(y = exp(mean_prediction)), col = "coral")
