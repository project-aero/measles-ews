...
b <- predict(smooth.spline(x=biweek_data$biweek, y=biweek_data$births), x=biweek_data$biweek)$y
N <- predict(smooth.spline(x=biweek_data$biweek, y=biweek_data$population), x=biweek_data$biweek)$y
ntimes <- nrow(biweek_data)
nobs <- nrow(biweek_data)
nseas <- 26
s <- rep(1:nseas, 11)
base <- periodic.bspline.basis(x = 1:26, nbasis = 6, degree = 3, period = 26)
trend <- biweek_data$biweek - mean(biweek_data$biweek)
...
mcmc_results <- clusterEvalQ(
  cl, {
    ...
    vars_to_collect <- c(
      "Iobs", "I", "S", "beta_mu", "beta", "beta_season", "rho", "inv_lambda", 
