...
$$
y_t \sim \text{Negative Binomial}(\rho I_t, \eta_t) \\
...
model{
  ...
  for(i in 1:nobs){
    ...
    # error[i] <- 1/(rho*I[i] + some)^2
    # y[i] ~ dnorm(rho*I[i],  error[i])
    # y[i] ~ dpois(rho*I[i])
  }
  # some ~ dunif(0, 10)
  r ~ dunif(0, 50)
  ...
  # Process model
  for(t in 1:(ntimes-1)){
    escape_prob[t] = exp(-beta[t] * (I[t] + 1))
    escapees[t] ~ dbin(escape_prob[t], S[t])
    I[t+1] = max(0.0000001, S[t] - escapees[t])
  ...
  for(t in 2:ntimes){
    ...
    gamma[t] = psi*gamma[t-1] + (1-psi)*noise[t]
  }
  
  noise[1] ~ dnorm(0, tau_gamma)
  gamma[1] = psi*exp(gamma0) + (1-psi)*noise[1]
  gamma0 ~ dunif(-15, -9)  # prior ranging from Re = 1 to Re = 25
  tau_gamma = pow(sigma_gamma, -2)
  ...
  # Initial conditions
  S0 ~ dunif(200000, 700000)
  S02 ~ dpois(S0)
  S[1] <- S02
  I0 ~ dpois(initI * (1+rho))
  I[1] = I0

  # Derived quantities
  for(i in 1:nobs){
    Iobs[i] = I[i]*rho 
    Rnaught[i] = gamma[i]*N[i]
  }
}
"
```
...
mcmc_results <- coda.samples(
  model, 
  variable.names = c("Iobs", "I", "S", "Rnaught", "gamma", "beta", "rho", "escape_prob", "psi", "theta"), 
  n.iter = 5000, 
  n.thin = 2
)
...
gamma <- ggplot(transmission_rate, aes(x = date, y = median_value)) +
  ...
  labs(x = "Date", y = expression(italic(R[t])))
