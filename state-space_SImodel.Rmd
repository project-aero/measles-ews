...
model{
  # Likelihood/Data Model
  for(i in 1:nobs){
    # y[i] ~ dnorm(rho * I[i], rho * I[i] + tau)  # approximates overdispersed Poisson
    y[i] ~ dpois(rho * I[i])
  }
  tau = pow(sigma, -2)
  sigma ~ dunif(0, 10)
  ...
  gamma[1] = psi*mean_gamma + (1 - psi)*noise[1]
  mean_gamma ~ dgamma(1, 1000)
  tau_gamma = pow(sigma_gamma, -2)
...
mcmc_results <- coda.samples(
  model, 
  variable.names = c("Iobs", "I", "S", "Rnaught", "beta", "rho"), 
  n.iter = 2000, 
  n.thin = 1
)
...
gamma <- ggplot(transmission_rate, aes(x = date, y = median_value)) +
  ...
  labs(x = "Date", y = "Transmission rate")
