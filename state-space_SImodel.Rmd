...
Here, I will implement a more flexible approach, where seasonality is incorporated with a sin wave and year-specific noise.
...
source("./ferrari_results_2008/niameyMCMC.1.q")
...
```{r jags-model2, eval = FALSE, echo = FALSE}
measles_model <- "
model{
  # Likelihood/Data Model
  for(i in 1:nobs){
...
effective_rep_rate <- mcmc_tbl %>%
  filter(grepl("Rnaught\\[", Parameter)) %>%
  mutate(
    date = biweek_data$date
  )

tranmission_rate <- mcmc_tbl %>%
...
gamma <- ggplot(effective_rep_rate, aes(x = date, y = median_value)) +
