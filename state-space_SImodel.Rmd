...
model{
  ...
  for(t in 2:ntimes){
    ...
    # gamma[t] = intercept + base * gamma[t-1] + noise[t]
  }
...
mcmc_results <- coda.samples(
  model, 
  variable.names = c("Iobs", "I", "S", "Rnaught", "gamma", "beta", "rho", "escape_prob", "psi", "psi2", "theta", "r", "sigma_gamma","epsilon", "S02", "I0", "gamma0", "m"), 
  n.iter = 200000, 
  n.thin = 10
)
...
exp_func <- function(base, intercept, start_value, times){
  outs <- numeric(times)
  outs[1] <- start_value
  for(t in 2:times){
    outs[t] <- intercept + base * outs[t-1] + rnorm(1,0,2)
  }
  return(outs)
} 


test <- exp_func(intercept = -7, base = 0.542, start_value = -10.6, times = 50)
