# analyze-mvw-elimination-sims.R
...
for(do_city in unique(all_sims$city)){
  for(do_speed in unique(all_sims$vacc_speed)){
    ...
    times_keep <- start_ts:length(times_before_vaccine)
    times_before_vaccine <- times_before_vaccine[times_keep]
...
write.csv(x = auc_tbl, "../results/elimination-mvw-grid-aucs.csv")
