...
by_sim <- model_sims %>% group_by(city, speed, sim) %>% nest()

sim_model <- function(df){
  loess(S ~ time, span = 52 / (nrow(df) * 2), data = df)
}

by_sim <- by_sim %>% mutate(model = purrr::map(data, sim_model))

by_sims <- by_sim %>% 
  mutate(pred = purrr::map2(data, model, modelr::add_predictions))

preds <- unnest(by_sims, pred)
