...
tst <- unnest(sims, data) %>% filter(speedf == "1.5e-05" & city == "Niamey") %>% 
  mutate(vacc_prog = ifelse(time < 50, "steady", "increasing")) %>%
...
foo3 <- foo2 %>% filter(epiid < 0) %>% group_by(sim, epiid, vacc_prog) %>% filter(is.finite(Shat_raw)) %>% summarise(covst = cov(time, Shat_raw),
                                                                                                    vart = var(time))

foo4 <- foo3 %>% group_by(vacc_prog, sim) %>% 
  summarise(tot_cov = sum(covst, na.rm = TRUE),
            tot_var = sum(vart, na.rm = TRUE),
            slope = tot_cov / tot_var)

foob <- foo %>% left_join(foo4, by = "vacc_prog")
