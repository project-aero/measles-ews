...
sizesum3 %>% group_by(speedf, city) %>% 
  ...
  ggplot(aes(fill = speedf, y = iepAUC, x = city)) + 
...
sizesum3 %>% group_by(speedf, city) %>% 
  summarise(epi_sizeAUC = calc_auc(predictions = epi_sizes, 
                              is_null = start_vacc == "initial vacc")) %>% 
  ggplot(aes(fill = speedf, y = epi_sizeAUC, x = city)) + 
  geom_bar(stat = "identity", position = "dodge")
```
...
tst %>% group_by(city, speedf) %>% 
  ...
  ggplot(aes(fill = speedf, y = mean_casesAUC, x = city)) + 
  geom_bar(stat = "identity", position = "dodge")
...
If vaccination is reducing the susceptible recruitment rate, we expect recent epidemics to more strongly inhibit current epidemics. We can quantify this inhibitory effect as the mean number of cases conditional on the recent epidemic history. To do this, we average the top ten highest $R_0$ values for each city. We convert these to numbers to effective reproduction numbers by multiplying by 0.3, the equilibrium fraction of susceptibles under the initial, steady vaccination program. We then computer the expected epidemics sizes as $N (1 -\exp(- R_E) )$, making use of the standard final size approximation for an epidemic in a closed population when the reproduction number is much greater than one.
