...
filtcalc <- function(df, filter_len = 1){
...
tsf <- unnest(sims, data) %>% filter(time > wstart & time < wstop) %>%
  group_by(city, speedf, sim) %>% nest() %>% 
  mutate(filtdf = purrr::map(data, filtcalc))
...
tsf2 %>% unnest(inhibests) %>% 
  ...
  group_by(city, speedf) %>% 
  summarise(inhib_AUC = mean(estimate > 0)) -> tsf3

tsf3 %>% ggbar(inhib_AUC)
