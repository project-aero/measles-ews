...
sizesum <- unnest(simfitred, iepsum) %>% 
...
sizegam3 <- mgcv::gam(
  (epi_sizes) ~ s(sfin, by = cityf) + s(start_season) + vacc_program + cityf, 
  data = sizesum, family = Gamma(link = log))
...
The variance-mean relationship rules out a standard gaussin model with identiy link function, so we try log transform, 4th-root transform, gamma log-link models. The gamma model seems to have the best diagnostic plots.
