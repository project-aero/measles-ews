...
tst <- unnest(sims, data) %>% filter(speedf == "1.5e-05" & city == "Niamey") %>% 
 ...
 mutate(Ihat = cases / (7 / 5)) %>% 
  ...
  mutate(Shat_raw = (lead(cases, 2) - eta * dt) * Nequil / (Ihat * dt * beta),
         Shat_filt = ifelse(I > 500, Shat_raw, NA))

foo <- filter(tst, sim == 1)


#dygraph(foo[, c("time", "S", "I", "beta", "Shat_filt")]) %>% dyRangeSelector()

naenc <- rle(is.finite(foo$Shat_filt))

naenc$values[naenc$values] <- seq_len(sum(naenc$value))
naenc$values[naenc$values == 0] <- -1 * seq_len(sum(naenc$value == 0))
foo$epiid <- inverse.rle(naenc)

foo3 <- foo %>% filter(epiid < 0) %>% group_by(epiid, vacc_prog) %>% filter(is.finite(Shat_raw)) %>% summarise(covst = cov(time, Shat_raw),
                                                                                                    vart = var(time))

foo4 <- foo3 %>% group_by(vacc_prog) %>% 
