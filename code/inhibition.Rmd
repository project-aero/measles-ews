...
To better understand the behavior of the model in the epidemic phases, we develop a statistical model of the amount by which S drops in these phases. 
...
tst <- unnest(sims, data) %>% filter(time > wstart & time < wstop) %>%
  ...
  summarize(mean_cases = mean(cases), mean_reports = mean(reports))
...
mean_AUC_df <- tst %>% group_by(city, speedf) %>% 
  summarise(mean_casesAUC = calc_auc(predictions = -mean_cases,
                                     is_null = vacc_prog == "steady"),
            mean_reportsAUC = calc_auc(predictions = -mean_reports,
                                       is_null = vacc_prog == "steady"))

mean_AUC_df %>% ggbar(depvar = mean_casesAUC)
mean_AUC_df %>% ggbar(depvar = mean_reportsAUC)
...
Can we estimate the relative rate of susceptible increase directly?
...
nu <- 0.05
gamma <- 365 / 5
denom <- eta / ((eta + nu) * (gamma + nu))

tst <- unnest(sims, data)  %>%
  filter(time > wstart & time < wstop) %>%
  group_by(speedf, city) %>%
  mutate(vacc_prog = ifelse(time < 50, "steady", "increasing")) %>%
  mutate(Ihat = cases / (7 / 5)) %>% 
  mutate(beta = R0_seas / denom) %>% 
  mutate(betabar = mean(beta)) %>%
  mutate(Shat_raw = (lead(cases, 2) - eta * dt) * Nequil / (Ihat * dt * betabar),
         Shat_filt = ifelse(I > 500, Shat_raw, NA))

foo <- filter(tst, sim == 5)
foo <- tst %>% group_by(sim, speedf, city) %>% nest()

#dygraph(foo[, c("time", "S", "I", "beta", "Shat_filt")]) %>% dyRangeSelector()
assign_epiid <- function(df){
  naenc <- rle(is.finite(df$Shat_filt))
  naenc$values[naenc$values] <- seq_len(sum(naenc$value))
  naenc$values[naenc$values == 0] <- -1 * seq_len(sum(naenc$value == 0))
  df$epiid <- inverse.rle(naenc)
  df
}

foo2 <- foo %>% mutate(data = purrr::map(data, assign_epiid)) %>% unnest()

foo3 <- foo2 %>% filter(epiid < 0) %>% group_by(sim, epiid, vacc_prog, speedf, city) %>% 
  filter(is.finite(Shat_raw)) %>% summarise(covst = cov(time, Shat_raw),                                                                                                                       vart = var(time))

foo4 <- foo3 %>% group_by(city, vacc_prog, sim, speedf) %>% 
  summarise(tot_cov = sum(covst, na.rm = TRUE),
            tot_var = sum(vart, na.rm = TRUE),
            slope = tot_cov / tot_var)

slope_AUC_df <- foo4 %>% group_by(city, speedf) %>% 
  summarise(slope_AUC = calc_auc(predictions = -slope,
                                is_null = vacc_prog == "steady"))

slope_AUC_df %>% ggbar(depvar = slope_AUC)

```

```{r plot-est-slopes}

foob <- foo2 %>% left_join(foo4, by = c("vacc_prog", "sim", "speedf", "city"))

foo5 <- foob %>% filter(epiid < 0) %>% group_by(epiid, vacc_prog, sim, speedf, city) %>% 
  mutate(lm = (time - mean(time)) * slope + median(Shat_raw))

foo5 %>% filter(city == "Niamey" & speedf == "4.9e-05" & sim == 5) %>% 
  ggplot(aes(x = time, y = S)) + geom_point() + geom_line(aes(y = lm))


ggsfit <- function(df = foo5, the_city = "Niamey", the_speedf = "4.9e-05", 
                   the_sim = 5){
  lab <- paste0(the_city, ", ramp up: ", the_speedf, " rep:", the_sim)
  df %>% filter(city == the_city & speedf == the_speedf & sim == the_sim) %>% 
    ggplot(aes(x = time, y = S)) + geom_point() + geom_line(aes(y = lm)) +
    ggtitle(label = lab)
  }

ggsfit(the_speedf = "1.5e-05")
ggsfit(the_speedf = "3.2e-05")
ggsfit(the_speedf = "4.9e-05")
ggsfit(the_speedf = "6.6e-05")
ggsfit(the_speedf = "8.3e-05")
ggsfit(the_speedf = "1e-04")
...
```{r eval = FALSE}
 ...
 foob <- foo2 %>% left_join(foo4, by = "vacc_prog")


foo5 <- foob %>% filter(epiid < 0) %>% group_by(epiid, vacc_prog) %>% 
  mutate(lm = (time - mean(time)) * slope + median(Shat_raw))

foo5 %>% ggplot(aes(x = time, y = S)) + geom_point() + geom_line(aes(y = lm))

tmpf <- function(x){
  if(nrow(x) == 1 ){
    x$Shat_mod <- x$Shat_filt
  } else {
    x$Shat_raw[!is.finite(x$Shat_raw)] <- NA
    m <- try(lm(Shat_raw~time, data = x))
    if (inherits(m, "try-error")) {
      browser()
    }
    x$Shat_mod <- predict(m, newdata = data.frame(time = x$time))
  }
  x
}

spll <- lapply(spl, tmpf)

foo2 <- bind_rows(spll) %>% arrange(time)

dygraph(foo2[, c("time", "S", "I", "beta", "Shat_filt", "Shat_mod")]) %>% dyRangeSelector()
...
filtcalc <- function(df, filter_len = 3){
  ...
  df %>% mutate(year = round(time)) %>%
    group_by(year) %>% 
    summarise(cases_yr = sum(cases), reports_yr = sum(reports)) %>% 
    mutate(cases_filt = filtfun(cases_yr),
           cases_yrnxt = lead(cases_yr, 1),
           reports_filt = filtfun(reports_yr),
           reports_yrnxt = lead(reports_yr, 1), 
           vacc_prog = ifelse(year < 50, "steady", "increasing")) %>% 
    select(year, cases_yrnxt, cases_filt, reports_yrnxt, 
           reports_filt, vacc_prog) %>%
    filter(year > df$wstart[1] & year < df$wstop[1])
}

inhib_model_cases <- function(int, df){
  lm(log(int - cases_yrnxt) ~ 0 + cases_filt + cases_filt:vacc_prog, data = df)
}

inhib_model_reports <- function(int, df){
  lm(I(int - reports_yrnxt) ~ 0 + reports_filt + reports_filt:vacc_prog, data = df)
}
...
tsf2 <- intercepts %>% select(city, base_size) %>% 
  ...
  ungroup() %>%
  mutate(inhibm_cases = purrr::map2(base_size, filtdf, inhib_model_cases),
         inhibm_reports = purrr::map2(base_size, filtdf, inhib_model_reports),
         inhibests_cases = purrr::map(inhibm_cases, broom::tidy),
         inhibests_reports = purrr::map(inhibm_reports, broom::tidy))

tsf2 %>% unnest(inhibests_cases) %>% 
  filter(term == "cases_filt:vacc_progincreasing") %>%
  group_by(city, speedf, flen) %>% 
  summarise(inhib_cases_AUC = mean(estimate > 0)) -> tsf3

tsf3 %>% ggbar(inhib_cases_AUC) + geom_hline(yintercept = 0.8) + facet_grid(flen~.)

```

It seems that the filter length has a relatively minor effect. Also, the AUCs are very similar to those of the mean.

```{r compare-mean-inhib}

tsf3 %>% filter(flen == 4) %>% 
  inner_join(mean_AUC_df, by = c("city", "speedf")) %>%
  mutate(meanAUCcomp = 1 - mean_casesAUC) %>% 
  select(city, speedf, inhib_cases_AUC, meanAUCcomp) %>% 
  gather("inhib_cases_AUC", "meanAUCcomp", key = "indicator", value = "AUC") %>%
  ggplot(aes(x = speedf, y = AUC, color = indicator)) + 
  geom_point() + facet_grid(city ~ .) + ggtitle("Filter length == 4")
...
tsf3 %>% filter(flen == 20) %>% 
  ...
  gather("inhib_cases_AUC", "meanAUCcomp", key = "indicator", value = "AUC") %>%
  ggplot(aes(x = speedf, y = AUC, color = indicator)) + 
  geom_point() + facet_grid(city ~ .) + ggtitle("Filter length == 20")



```

The inhibition indicator achieves slightly better performance than the mean for Zinder and Agadez at high speeds. 

# Effect of reporting probability

```{r}

tsf2 %>% unnest(inhibests_reports) %>% 
  filter(term == "reports_filt:vacc_progincreasing") %>%
  group_by(city, speedf, flen) %>% 
  summarise(inhib_reports_AUC = mean(estimate > 0)) -> tsf3r

tsf3r %>% ggbar(inhib_reports_AUC) + geom_hline(yintercept = 0.8) + 
  facet_grid(flen~.)
...
tsf3r %>% filter(flen == 20) %>% 
  inner_join(mean_AUC_df, by = c("city", "speedf")) %>%
  mutate(meanreportsAUCcomp = 1 - mean_reportsAUC) %>% 
  select(city, speedf, inhib_reports_AUC, meanreportsAUCcomp) %>% 
  gather("inhib_reports_AUC", "meanreportsAUCcomp", 
         key = "indicator", value = "AUC") %>%
  ggplot(aes(x = speedf, y = AUC, color = indicator)) + 
  geom_point() + facet_grid(city ~ .) + ggtitle("Filter length == 20")
...
The results with reporting error are similar to the case of zero error. AUCs are slightly lower for both indicators.
