...
plot_iep_dist <- function(df){
  g <- ggplot(data = df, aes(x = iep_weeks, color = start_vacc)) + 
    geom_density() + 
    facet_grid(as.numeric(speed) ~ city, scales = "free_y")
  plot(g)
  return(0)
}
...
sizesum3 %>% group_by(speed, city) %>% 
  summarise(iepAUC = calc_auc(predictions = epi_sizes, 
                              is_null = start_vacc == "initial vacc"))

```

The abundance of AUC statistics below 0.5 confirm and summarise that the interepidemic periods are indeed typically shorter in the "null" period in which the vaccination rate stays at 0.7.
