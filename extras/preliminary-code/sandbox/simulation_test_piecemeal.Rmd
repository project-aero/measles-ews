...
model{
  ...
  for(i in 1:epiyears){
    ...
    I[1, i] <- trunc(I0[i])
    S[1, i] <- trunc(S0[i])
    p[1, i] <- eta/(eta + (rho*I[1, i]))
    Y[1, i] ~ dnegbin(p[1, i], eta)

    for(j in 1:(biweeks-1)){
      epsilon[j, i] ~ dnorm(0, 1)
      gamma[j, i] <- gamma0 + epsilon[j, i]
      beta[j, i] <- exp(gamma[j, i]) * (1 + season[j])
      lambda[j, i] <- exp(-beta[j, i] * (I[j, i] + psi))
      Delta[j, i] ~ dbin(lambda[j, i], S[j, i])
      I[j+1, i] <- S[j, i] - Delta[j, i]
      S[j+1, i] <- b + Delta[j, i]
      p[j+1, i] <- eta/(eta + (rho*I[j+1, i]))
      Y[j+1, i] ~ dnegbin(p[j+1, i], eta)
    }
  }
...
sim_out <- sim_out %>%
  mutate(
    biweek = rep(1:26, times = max(year))
  )

Y <- sim_out %>%
  dplyr::select(year, biweek, case_reports) %>%
  spread(year, case_reports) %>%
  dplyr::select(-biweek) %>%
  as.matrix()

epiyears <- length(unique(sim_out$year))
biweeks <- length(unique(sim_out$biweek))
knots <- 5
bases <- bSpline(1:biweeks, df = knots, degree = 2, intercept = TRUE)
births <- unique(b)
```
...
mcmc_summ <- summary(mcmc_results)$quantiles
Ipreds <- mcmc_summ[grep("Iobs", rownames(mcmc_summ)), 3]
Ilow <- mcmc_summ[grep("Iobs", rownames(mcmc_summ)), 1]
Ihi <- mcmc_summ[grep("Iobs", rownames(mcmc_summ)), 5]
plot(Ihi,type = "l",lty=2)
lines(Ilow, lty = 2)
lines(Ipreds, lty=1)
points(sim_out$case_reports)

Ipreds <- mcmc_summ[grep("I", rownames(mcmc_summ)), 3]
Ipreds <- Ipreds[-grep("Iobs", names(Ipreds))]
Ipreds <- Ipreds[-grep("I0", names(Ipreds))]
