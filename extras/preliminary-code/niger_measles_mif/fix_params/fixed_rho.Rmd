  ...
  t1 <- system.time({
    m1 <- foreach(i=1:20,.packages='pomp',.combine=rbind,
      ...
      mf <- mif2(measles_pomp,
                 ...
                 transform=TRUE,
                 rw.sd=rw.sd(
                   beta_mu=0.02, tau=0.01, rho = 0.02, sigma_env = 0.02,
                   b1 = 0.02, b2 = 0.02, b3 = 0.02, b4 = 0.02, b5 = 0.02, b6 = 0.02,
                   I_0=ivp(0.1), psi = 0.02
                 )
                )
...
Now time to estimate the states with empirical Bayes.
...
reff_time <- mcmcout %>% 
  ...
  mutate(
    ...
    population = rep(biweek_data$population, each = length(unique(rep)))
  ) %>%
...
ggplot(reff_time, aes(x = date, y = median_value)) +
