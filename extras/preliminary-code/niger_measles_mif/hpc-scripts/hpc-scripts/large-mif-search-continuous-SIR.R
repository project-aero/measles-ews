...
biweek_data <- readRDS("weekly_data_cleaned.RDS")

if(file.exists("measles-pomp.RDS")){
  measles_pomp <- readRDS("measles-pomp-continuous_SIR.RDS")
} else{
  source("define-measles-pomp-continuous-SIR.R")
  measles_pomp <- readRDS("measles-pomp-continuous_SIR.RDS")
}
...
stew(
  file = "probe-match-results-cont.rda", {
    pm <- probe.match(
      measles_pomp,
      probes = plist,
      est = param_names,
