...
heat_plot <- ggplot(niger_measles, aes(x = date, y = region, fill = log10(cases+1))) +
  geom_tile() +
  scale_fill_viridis() +
...
nblist <- nb2listw(regions_nb, style = "B")
...
This first requires defining "epidemic year," which I will define as October to September (see figure below).

```{r epi-years}
total_cases_inspace <- niger_measles_inspace %>%
  filter(region == "Niamey") %>%
  group_by(date) %>%
  summarise(total_cases = sum(cases, na.rm = TRUE))
...
morans_kendall <- morans_over_time %>%
  filter(is.na(runmean_two_year) == FALSE) %>%
  left_join(epidemic_timings, by = "date") %>%
  group_by(epidemic_year) %>%
  summarise(
    kens_tau = cor(runmean_two_year, epidemic_week, method = "kendall"),
    kens_test = cor.test(runmean_two_year, epidemic_week, method = "kendall", alternative = "greater")$p.value
  )

peak_cases <- total_cases_inspace %>%
  group_by(epidemic_year) %>%
  summarise(
    peak = max(total_cases)
  )

plot(morans_kendall$kens_tau, peak_cases$peak[4:(nrow(peak_cases))])
summary(lm(peak_cases$peak[4:(nrow(peak_cases))] ~ morans_kendall$kens_tau))
```
