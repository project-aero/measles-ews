...
TODO: Make transmission periodic such that each biweek gets its own beta, but those biweek betas are constant across years.
...
model{
  # Latent process
  for(t in 1:(weeks-1)){
    log(lambda[t]) <- log(beta[win[t]]) + alpha*log(I[t]) + log(z[t] + Sbar)  # epidemic intensity
...
#   mutate(biweek = rep(niamey_jags$biweek, times = 1))
# 
# ggplot(mcmc_df[1:286,], aes(x = biweek, y = mean_value)) +
#   geom_point(color = "red")+
#   # geom_point(data = niamey_jags, aes(y = cases))+
